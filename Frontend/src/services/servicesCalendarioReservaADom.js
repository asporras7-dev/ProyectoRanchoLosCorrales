...
export const getFechasBloqueadasADomicilio = async () => {
    try {
        const response = await fetch(`${API_URL}/fechas-bloqueadas-a-domicilio`);
 ...
 */
export const bloquearFechaADomicilio = async (fecha) => {
    try {
        const response = await fetch(`${API_URL}/fechas-bloqueadas-a-domicilio`, {
            method: "POST",
 ...
 * @param {string} fecha - "YYYY-MM-DD"
 */
export const desbloquearFechaADomicilio = async (fecha) => {
    try {
        const response = await fetch(`${API_URL}/fechas-bloqueadas-a-domicilio/${fecha}`, {
            method: "DELETE",
        });
        const data = await response.json();
        if (!response.ok) {
